...
\section{Datenbank und ORM-Technologien}

Die Entwicklung moderner Webanwendungen erfordert fundierte Entscheidungen bezüglich der Datenpersistenz und serverseitigen Logik. Die folgenden Abschnitte analysieren relevante Technologien: PostgreSQL als relationales Datenbanksystem, Edge Functions für serverlose Compute-Workloads, Realtime-Systeme für Event-basierte Synchronisation sowie Object-Relational Mapping als Abstraktionsschicht.

\subsection{PostgreSQL als relationales Datenfundament}

PostgreSQL ist ein objektrelationales Datenbanksystem mit über 35 Jahren aktiver Entwicklung, das sich durch Zuverlässigkeit, Feature-Robustheit und Performance auszeichnet~\cite{PostgreSQLAbout2024}. Als Open-Source-Projekt unter der PostgreSQL-Lizenz bietet es eine kostenfreie Alternative zu kommerziellen Datenbanksystemen bei vergleichbarem Funktionsumfang.

\textbf{Erweiterte Datentypen:} PostgreSQL unterstützt neben Standard-SQL-Typen auch JSONB für semi-strukturierte Daten, Arrays, geometrische Typen und benutzerdefinierte Typen~\cite{PostgreSQLAbout2024}. JSONB ermöglicht die Speicherung und Indizierung von JSON-Dokumenten, wodurch hybride Datenmodelle realisierbar sind, die relationale Strukturen mit dokumentenorientierten Anteilen kombinieren.

\textbf{Fortgeschrittene Abfragefähigkeiten:} Common Table Expressions (CTEs) ermöglichen rekursive Abfragen, die für die Traversierung hierarchischer Datenstrukturen wie Baumstrukturen oder gerichteter Graphen essentiell sind. Window Functions bieten analytische Berechnungen über Zeilenmengen hinweg ohne Gruppierung. Foreign Key Constraints mit Cascade-Optionen gewährleisten referentielle Integrität über Tabellengrenzen hinweg~\cite{PostgreSQLAbout2024}.

\textbf{Managed PostgreSQL-Dienste:} Cloud-Anbieter wie Supabase, AWS RDS oder DigitalOcean stellen vollwertige PostgreSQL-Instanzen mit automatisierten Backups, Connection Pooling und Monitoring bereit~\cite{SupabaseGitHub2024}. Diese Managed Services reduzieren den Administrationsaufwand, während das standardisierte SQL-Interface die Portabilität zwischen Anbietern sicherstellt.

\subsection{Edge Functions: Serverless Computing am Netzwerkrand}

Edge Functions repräsentieren eine Weiterentwicklung des Serverless-Paradigmas, bei der Code nicht in zentralisierten Rechenzentren, sondern an geografisch verteilten Edge-Standorten nahe der Nutzer ausgeführt wird~\cite{SupabaseDocsEdgeFunctions2024}.

\textbf{Technische Architektur:} Supabase Edge Functions basieren auf Deno, einem modernen JavaScript/TypeScript-Runtime mit nativer TypeScript-Unterstützung ohne Transpilationsschritt~\cite{SupabaseDocsEdgeFunctions2024}. Funktionen werden als einzelne TypeScript-Dateien definiert, die einen HTTP-Handler exportieren. Das Deployment erfolgt durch Bundling in ein kompaktes ESZip-Format, wobei eine eindeutige URL für den Aufruf generiert wird.

\textbf{Ausführungsmodell:} Edge Functions nutzen ein Isolate-basiertes Ausführungsmodell. Isolates sind leichtgewichtige Ausführungsumgebungen, die schneller starten als Container oder virtuelle Maschinen. Selbst initiale Ausführungen erreichen Latenzzeiten im Millisekundenbereich. Isolates können für einen Zeitraum aktiv bleiben, um nachfolgende Requests ohne Neustart zu handhaben, und mehrere Isolates können gleichzeitig am selben Edge-Standort laufen~\cite{SupabaseDocsEdgeFunctions2024}.

\textbf{Anwendungsfälle:} Typische Einsatzszenarien umfassen Webhooks, periodische Datenimporte via Cron-Trigger, Datenvalidierung und -transformation sowie die Integration externer APIs. Die serverlose Architektur eliminiert die Notwendigkeit permanent laufender Server für sporadische Aufgaben.

\subsection{Realtime: Event-basierte Datensynchronisation}

Supabase Realtime ermöglicht das Lauschen auf PostgreSQL-Datenbankänderungen und deren Broadcast über WebSockets an verbundene Clients~\cite{SupabaseGitHubRealtime2024}. Diese Fähigkeit adressiert Anwendungsfälle, bei denen mehrere Clients synchronisierte Datenansichten benötigen.

\textbf{Architektur:} Das Realtime-System ist ein global verteilter Elixir-Cluster, der auf dem Phoenix Framework aufbaut~\cite{SupabaseGitHubRealtime2024}. Die Kernfunktionalität nutzt PostgreSQL's Logical Replication: Datenbankänderungen werden abgefangen, zu JSON konvertiert und über WebSocket-Channels an autorisierte Clients verteilt. Phoenix Channels implementieren ein Publish-Subscribe-Modell, das Millionen gleichzeitiger Verbindungen durch Elixirs leichtgewichtige Prozesse handhaben kann.

\textbf{Funktionale Komponenten:} Das System bietet drei Hauptfunktionen~\cite{SupabaseGitHubRealtime2024}: \textit{Broadcast} ermöglicht das Senden ephemerer Nachrichten von Client zu Clients mit niedriger Latenz. \textit{Presence} ermöglicht Tracking und Synchronisation von geteiltem State zwischen Clients, etwa für Anwesenheitsanzeigen. \textit{Postgres Changes} ermöglicht Event-basiertes Lauschen auf INSERT-, UPDATE- und DELETE-Operationen mit Schema- und Tabellen-Targeting sowie granularem Filtering.

\subsection{Object-Relational Mapping mit Drizzle ORM}

\ac{ORM} ist eine Programmiertechnik zur Konvertierung von Daten zwischen relationalen Datenbanken und objektorientierten Programmiersprachen~\cite{WikipediaORM2024}. Drizzle ORM positioniert sich als TypeScript-First-Lösung, die SQL nicht abstrahiert, sondern in einer typisierten API widerspiegelt~\cite{DrizzleORMDocs2024}.

\textbf{Philosophie:} Die Leitlinie \glqq If you know SQL, you know Drizzle\grqq{} unterscheidet Drizzle von traditionellen ORMs~\cite{DrizzleORMDocs2024}. Statt SQL hinter Abstraktionsschichten zu verbergen, bietet Drizzle einen typisierten SQL-Builder, dessen Queries für Entwickler mit SQL-Kenntnissen unmittelbar verständlich bleiben.

\textbf{Schema-Definition:} Das Datenbankschema wird deklarativ in TypeScript definiert, woraus Drizzle automatisch Typen für Queries und Ergebnisse inferiert~\cite{DrizzleORMDocs2024}. Im Gegensatz zu Prisma erfordert Drizzle keinen Code-Generierungsschritt nach Schemaänderungen -- die Typen sind zur Entwicklungszeit sofort verfügbar.

\textbf{Type Safety:} TypeScripts statische Typprüfung stellt sicher, dass Queries konsistent mit dem definierten Schema sind~\cite{LogRocketDrizzle2024}. Tippfehler bei Spaltennamen oder Typinkompatibilitäten werden zur Compile-Zeit erkannt, nicht erst zur Laufzeit. Die Bibliothek operiert ohne Runtime-Overhead -- Queries werden zur Build-Zeit in SQL-Strings kompiliert und hat keine externen Dependencies~\cite{DrizzleORMDocs2024}.


\textbf{Performance-Charakteristika:} Drizzle operiert ohne Runtime-Overhead eines Query-Builders zur Laufzeit -- Queries werden zur Build-Zeit in SQL-Strings kompiliert~\cite{DrizzleORMDocs2024}. Die Bibliothek hat keine externen Dependencies, was die Bundle-Größe minimiert und Edge-Deployments ermöglicht.
