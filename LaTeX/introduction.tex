...
Diese Operationalisierung adressiert das in Abschnitt \ref{sec:problemstellung} beschriebene Fehlen offizieller Planungstools und nutzt die von der Community bereitgestellten Daten als Grundlage. Sie bildet die methodische Basis für die nachfolgend beschriebene Entwicklung einer Companion App.

\section{Entwicklungsmethodik und Phasenmodell}\label{sec:entwicklungsmethodik}

\subsection{Phase 1: Requirements Engineering}\label{sec:phase1}
...
\begin{itemize}
  ...
  \item \textbf{3.1.2 Continuous Integration:} Automatisierte Builds bei jedem Commit über GitHub Actions oder ähnliche CI-Tools sowie automatisierte Testausführung der gesamten Test-Suite. Code Quality Checks umfassen Linting (ESLint für TypeScript). Automatisches Deployment auf Staging-Umgebung für frühzeitiges Testen.
\end{itemize}
...
\begin{itemize}
  \item \textbf{4.1.1 Performance-Metriken:} Messung von Page Load Time durch Metriken wie der \ac{TTI} für alle Hauptseiten sowie aller Akzeptanzkriterien mit Performance-Anforderungen.
  
  \item \textbf{4.1.2 User-Engagement-Metriken:} Sofern Veröffentlichung erfolgt: Daily Active Users (DAU), Feature Usage Statistics zur Identifikation der am häufigsten genutzten Funktionalitäten sowie User Retention Rate zur Bewertung des langfristigen Nutzens.
\end{itemize}
...
\section{Methoden und Verfahren}\label{sec:methoden-verfahren}

Dieses Kapitel dokumentiert die methodischen Grundlagen sowie den eingesetzten Technologie-Stack der ArcDéx Companion App. Die vollständige Versionierung aller Komponenten gewährleistet die Replizierbarkeit der Ergebnisse.

\subsection{Methodische Grundlagen}

Die Entwicklung folgte einem agilen Vorgehensmodell, das die in Abschnitt~\ref{sec:entwicklungsmethodik} beschriebenen vier Phasen iterativ durchläuft. Die methodischen Kernelemente umfassen:

\textbf{User Stories nach INVEST-Kriterien:} Anforderungen wurden als User Stories formuliert, die den INVEST-Qualitätskriterien entsprechen: Independent (unabhängig priorisierbar), Negotiable (verhandelbar im Detail), Valuable (wertstiftend für Stakeholder), Estimable (schätzbar im Aufwand), Small (in einer Iteration umsetzbar) und Testable (durch Akzeptanzkriterien verifizierbar)~\cite{wake2003invest}. Das standardisierte Format \glqq Als [Rolle] möchte ich [Funktion], um [Nutzen] zu erreichen\grqq{} ermöglichte die konsistente Dokumentation aller funktionalen Anforderungen.

\textbf{Akzeptanzkriterien nach SMART-Prinzip:} Jede User Story wurde mit messbaren Erfolgskriterien versehen, die den SMART-Kriterien genügen (Specific, Measurable, Achievable, Relevant, Time-bound). Diese Akzeptanzkriterien bildeten die Grundlage für die automatisierte Testvalidierung in Phase~4.

\textbf{Iterative Entwicklung mit Continuous Integration:} Die Implementierung erfolgte in Iterationen mit kontinuierlicher Integration über GitHub Actions. Automatisierte Builds, Linting und Test-Ausführung, Preview Deployments über Vercel ermöglichten frühzeitiges Feedback und reduzierten Integrationsrisiken~\cite{VercelPreviewDocs2024}.

\textbf{MoSCoW-Priorisierung:} Die Anforderungen wurden nach der MoSCoW-Methode priorisiert (Must, Should, Could, Won't), wobei die Kernprobleme aus Abschnitt~\ref{sec:problemstellung} als Must-Anforderungen klassifiziert wurden~\cite{clegg1994case}.

\subsection{Verwendete Tools und Technologien}\label{sec:tools-technologien}

Der Technologie-Stack basiert auf dem T3 Stack als vorkonfigurierte Grundlage für typsichere Full-Stack-Entwicklung. Tabelle~\ref{tab:tech-stack} fasst die Kernkomponenten zusammen, die nachfolgenden Abschnitte erläutern die Technologieauswahl.

\begin{table}[htbp]
\centering
\caption{Technologie-Stack mit Versionen (Stand: Dezember 2025)}
\label{tab:tech-stack}
\begin{tabular}{llll}
\toprule
\textbf{Kategorie} & \textbf{Technologie} & \textbf{Version} & \textbf{Zweck} \\
\midrule
\multicolumn{4}{l}{\textit{Frontend Framework \& Runtime}} \\
& Next.js & 15.2.6 & React-Framework mit App Router \\
& React & 19.0.0 & Komponentenbasierte UI-Library \\
& TypeScript & 5.8.2 & Statische Typisierung \\
\midrule
\multicolumn{4}{l}{\textit{Styling \& UI-Komponenten}} \\
& Tailwind CSS & 4.0.15 & Utility-First CSS-Framework \\
& Radix UI & diverse & Barrierefreie UI-Primitives \\
& Framer Motion & 12.23.24 & Animationen \\
& Lucide React & 0.546.0 & Icon-Library \\
\midrule
\multicolumn{4}{l}{\textit{Visualisierung}} \\
& React Flow & 11.11.4 & Interaktive Graphen \\
& Dagre & 0.8.5 & Automatisches Graph-Layout \\
& Recharts & 2.15.4 & Diagramme und Charts \\
\midrule
\multicolumn{4}{l}{\textit{State Management}} \\
& Zustand & 5.0.8 & Client State mit Persistierung \\
& TanStack Query & 5.90.5 & Server State und Caching \\
\midrule
\multicolumn{4}{l}{\textit{Backend \& Datenbank}} \\
& Drizzle ORM & 0.40.1 & Typsicheres SQL-ORM \\
& PostgreSQL & -- & Relationale Datenbank (Supabase) \\
\midrule
\multicolumn{4}{l}{\textit{Qualitätssicherung}} \\
& Playwright & 1.57.0 & End-to-End Testing \\
& ESLint & 9.23.0 & Code-Linting \\
& Prettier & 3.5.3 & Code-Formatierung \\
\midrule
\multicolumn{4}{l}{\textit{Build \& Deployment}} \\
& pnpm & 10.14.0 & Package Manager \\
& Vercel & -- & Hosting mit Preview Deployments \\
\bottomrule
\end{tabular}
\end{table}
...
Das Frontend basiert auf Next.js 15 mit dem App Router für dateibasiertes Routing und hybrides Rendering~\cite{nextjsdocs2024}. Die vollständige TypeScript-Typisierung mit strikten Compiler-Optionen (\texttt{strict: true}, \texttt{noUncheckedIndexedAccess}) ermöglicht die frühzeitige Fehlererkennung während der Entwicklung~\cite{Djirdeh2024TypeScriptBestPractices}.

Die UI-Architektur kombiniert Radix UI Primitives für barrierefreie Basiskomponenten mit Tailwind CSS 4 für Utility-First Styling~\cite{WikipediaTailwindCSS}. React Flow realisiert die interaktiven Quest-Flow-Charts und Recycling-Visualisierungen, wobei der Dagre-Algorithmus automatische hierarchische Layouts berechnet~\cite{ReactFlowDocs2024Layouting}.

\subsubsection{State Management und Datenhaltung}

Die Trennung von Client State und Server State folgt etablierten Best Practices~\cite{DevCommunity2023ServerClientState}: Zustand verwaltet lokalen Anwendungszustand mit optionaler Local-Storage-Persistierung, während TanStack React Query serverseitigen State mit automatischem Caching und Revalidierung handhabt~\cite{TanStackQuery2024Overview}.

Die Datenbankanbindung erfolgt über Drizzle \ac{ORM}, das typsichere SQL-Abfragen ermöglicht und TypeScript-Typen automatisch aus dem Datenbankschema generiert~\cite{DrizzleORMDocs2024}. Die PostgreSQL-Datenbank wird über Supabase betrieben.

\subsubsection{Qualitätssicherung und Deployment}

Playwright ermöglicht browser-automatisierte \ac{E2E}-Tests zur Validierung der in Abschnitt~\ref{sec:phase1} definierten Akzeptanzkriterien. Die Code-Qualität wird durch ESLint mit TypeScript-spezifischen Regeln sowie Prettier für konsistente Formatierung sichergestellt.

Das Deployment erfolgt über Vercel mit automatischen Preview Deployments für jeden Git-Branch, was kollaboratives Review ohne separate Staging-Infrastruktur ermöglicht~\cite{VercelPreviewDocs2024}.
